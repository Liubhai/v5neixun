...
- (void)setCourseCollectionManagerModel:(ShopCarCourseModel *)model indexpath:(nonnull NSIndexPath *)indexpath {
    ...
    _learnCountLabel.text = [NSString stringWithFormat:@"%@人在学",model.sale_count];
    ...
    if ([priceValue isEqualToString:@"0.00"] || [priceValue isEqualToString:@"0.0"] || [priceValue isEqualToString:@"0"]) {
        ...
        _priceLabel.text = [NSString stringWithFormat:@"%@%@",IOSMoneyTitle,priceValue];
        _priceLabel.textColor = EdlineV5_Color.faildColor;
    }
...
- (void)setCourseCollectionRootManagerModel:(ShopCarCourseModel *)model indexpath:(NSIndexPath *)indexpath showSelect:(BOOL)showSelect {
    ...
    if (showSelect) {
        ...
        _courseFace.frame = CGRectMake(_selectedIconBtn.right + 10, 10, 153, 86);
    }
