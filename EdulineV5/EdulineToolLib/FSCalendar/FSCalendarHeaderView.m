...
- (void)initialize
{
    ...
    UIButton *todayButton = [[UIButton alloc] initWithFrame:CGRectMake(MainScreenWidth - 58, 0, 58, 20)];
    [todayButton setTitle:@"今天" forState:0];
    [todayButton setTitleColor:EdlineV5_Color.textFirstColor forState:0];
    todayButton.titleLabel.font = SYSTEMFONT(14);
    [todayButton addTarget:self action:@selector(backTodayButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:todayButton];
    _backTodayButton = todayButton;
}
...
- (void)layoutSubviews
{
    ...
    self.backTodayButton.frame = CGRectMake(self.fs_width - 58, self.fs_height*0.1, 58, self.fs_height*0.9);
}
...
- (void)scrollToOffset:(CGFloat)scrollOffset animated:(BOOL)animated
{
    if (self.scrollDirection == UICollectionViewScrollDirectionHorizontal) {
        CGFloat step = self.collectionView.fs_width*((self.scrollDirection==UICollectionViewScrollDirectionHorizontal)?1:1);
        [_collectionView setContentOffset:CGPointMake((scrollOffset+1)*step, 0) animated:animated];
    } else {
...
- (void)backTodayButtonClick:(UIButton *)sender {
    if (_delegate && [_delegate respondsToSelector:@selector(backToToday)]) {
        [_delegate backToToday];
    }
}
...
- (void)prepareLayout
{
    ...
    self.itemSize = CGSizeMake(
                               self.collectionView.fs_width*((self.scrollDirection==UICollectionViewScrollDirectionHorizontal)?1:1),
