...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        _deliver.hidden = YES;//self.calendarHeaderView.hidden;
...
// MARK: - 直播列表日历顶部左右翻页按钮点击事件
- (void)pageButtonClick:(BOOL)pageDirection {
    /**
     yes 右 NO 左
     */
    if (pageDirection) {
        _collectionView.contentOffset = CGPointMake(_collectionView.contentOffset.x + MainScreenWidth, 0);
//        NSDate *date = [self.calculator dateForIndexPath:[_collectionView indexPathForCell:_collectionView.visibleCells.lastObject]];
//        NSDate *lastDay = [NSDate dateWithTimeInterval:24*60*60*28 sinceDate:date];//后1天
//        [self scrollToDate:lastDay animated:YES];
    } else {
        _collectionView.contentOffset = CGPointMake(_collectionView.contentOffset.x - MainScreenWidth, 0);
//        NSDate *date = [self.calculator dateForIndexPath:[_collectionView indexPathForCell:_collectionView.visibleCells.firstObject]];
//        NSDate *lastDay = [NSDate dateWithTimeInterval:-24*60*60*28 sinceDate:date];//后1天
//        [self scrollToDate:lastDay animated:YES];
    }
}
...
- (void)reloadDataForCell:(FSCalendarCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.numberOfEvents = [_appearance.hasClassScheduleArray containsObject:[dateFormatter stringFromDate:date]] ? 1 : 0;
    switch (self.transitionCoordinator.representingScope) {
        case FSCalendarScopeMonth: {
