...
@interface CourseExamPopViewController ()<UITableViewDelegate, UITableViewDataSource, CourseExamPopCellDelegate> {
...
@property (strong, nonatomic) NSDictionary *currentExamInfoDict;

@property (strong, nonatomic) UIView *topWhiteView;

@end

@implementation CourseExamPopViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.layer.backgroundColor = [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.3000].CGColor;
    _examType = @"1";
    examNum = 0;
    ...
    CGFloat totalHeight = 364;
    
    CGFloat inSpace = 50.0;
    
    _topWhiteView = [[UIView alloc] initWithFrame:CGRectMake(inSpace, (MainScreenHeight - totalHeight) / 2.0, MainScreenWidth - inSpace * 2, 7)];
    _topWhiteView.backgroundColor = [UIColor whiteColor];
    CGFloat radius = 7; // 圆角大小
    UIRectCorner corner = UIRectCornerTopLeft | UIRectCornerTopRight; // 圆角位置
    UIBezierPath * path = [UIBezierPath bezierPathWithRoundedRect:_topWhiteView.bounds byRoundingCorners:corner cornerRadii:CGSizeMake(radius, radius)];
    CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
    maskLayer.frame = _topWhiteView.bounds;
    maskLayer.path = path.CGPath;
    _topWhiteView.layer.mask = maskLayer;
    ...
    _rightOrErrorIcon = [[UIImageView alloc] initWithFrame:CGRectMake(_topWhiteView.right - 74 - 4, _topWhiteView.top + 4, 74, 74)];
...
//    [self getData];
    [self getExamDetailForExamIds:_course_ID];
...
- (void)makeBottomView {
    if (!_bottomView) {
        _bottomView = [[UIButton alloc] initWithFrame:CGRectMake(50, _tableView.bottom, MainScreenWidth - 100, 50)];
        ...
        UIBezierPath * path = [UIBezierPath bezierPathWithRoundedRect:_bottomView.bounds byRoundingCorners:corner cornerRadii:CGSizeMake(radius, radius)];
        CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
        ...
        _previousExamBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, _bottomView.width, 50 - 7)];
        ...
        _nextExamBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, _bottomView.width, 50 - 7)];
    ...
    //        _nextExamBtn.titleEdgeInsets = UIEdgeInsetsMake(0, -imageWith-space/2.0, 0, imageWith+space/2.0);
            [EdulineV5_Tool dealButtonImageAndTitleUI:_nextExamBtn];
        }
        ...
        if (examCount == 1 || examNum == 0) {
            [_nextExamBtn setTitle:@"提交" forState:0];
            [_nextExamBtn setTitleColor:EdlineV5_Color.themeColor forState:0];
            [_nextExamBtn setImage:nil forState:0];
        }
        UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, 0, _bottomView.width, 1)];
...
- (void)makeTableView {
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(50, _topWhiteView.bottom, MainScreenWidth - 100, 364 - 50 - 7) style:UITableViewStyleGrouped];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *reuseId = @"CourseExamPopCell";
    CourseExamPopCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseId];
    if (!cell) {
        cell = [[CourseExamPopCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseId];
    }
    cell.examModuleType = NO;
    if (SWNOTEmptyArr(_examDetailArray)) {
        ExamDetailModel *model = [self checkExamDetailArray:currentExamId];
        if ([model.question_type isEqualToString:@"7"]) {
            [cell setCourseExamPopCellExamDetail:model cellModel:model.topics[indexPath.row] cellIndex:indexPath];
        } else {
            if (SWNOTEmptyArr(model.topics)) {
                ExamDetailModel *modelpass = model.topics[indexPath.section];
                [cell setCourseExamPopCellAnswerInfo:(ExamDetailOptionsModel *)(modelpass.options[indexPath.row]) mainExamDetail:model examDetail:modelpass cellIndex:indexPath];
            } else {
                [cell setCourseExamPopCellAnswerInfo:(ExamDetailOptionsModel *)(model.options[indexPath.row]) mainExamDetail:model examDetail:model cellIndex:indexPath];
            }
        }
    }
    cell.delegate = self;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (SWNOTEmptyArr(_examDetailArray)) {
        ...
        UITextView *lable1111 = [[UITextView alloc] initWithFrame:CGRectMake(11, 0, MainScreenWidth - 100 - 22, 20)];
        ...
        UIView *mediaBackView = [[UIView alloc] initWithFrame:CGRectMake(0, lable1111.bottom + 10, MainScreenWidth - 100, 0.01)];
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    ...
    if (SWNOTEmptyArr(_examDetailArray)) {
        ...
        return lable1111.height + 10 + model.material.count * (52 + 12) - 12;
    }
    return 0.001;
}
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    if ([_examType isEqualToString:@"3"]) {
        ...
        UIView *back = [[UIView alloc] initWithFrame:CGRectMake(0, 0, MainScreenWidth - 100, 1)];
        back.backgroundColor = [UIColor whiteColor];//EdlineV5_Color.backColor;
        
        
//        UILabel *examPointTitle = [[UILabel alloc] initWithFrame:CGRectMake(15, 12, back.width - (58 + 33) - 15, 20)];
...
//        [back setHeight:expandButton.bottom + 12];
        ...
        if (modelxxx.is_expand) {
            ...
            UILabel *rightAnswerTitle = [[UILabel alloc] initWithFrame:CGRectMake(15, 0, 75, 20)];
            ...
            UITextView *rightValueTextView = [[UITextView alloc] initWithFrame:CGRectMake(rightAnswerTitle.right - 10, rightAnswerTitle.top - 7, back.width - (rightAnswerTitle.right - 10) - 15, 20)];
            ...
            UITextView *analyzeTextView = [[UITextView alloc] initWithFrame:CGRectMake(keyTitle.right - 15, keyTitle.top - 7, back.width - keyTitle.right, 20)];
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section {
    if ([_examType isEqualToString:@"3"]) {
        ...
        if (modelxxx.is_expand) {
            UITextView *rightValueTextView = [[UITextView alloc] initWithFrame:CGRectMake(15 + 80, 0, MainScreenWidth - 100 - 95 - 15, 20)];
            rightValueTextView.attributedText = [[NSAttributedString alloc] initWithAttributedString:[[NSMutableAttributedString alloc] initWithString:SWNOTEmptyStr(modelxxx.examAnswer) ? modelxxx.examAnswer : @""]];
            rightValueTextView.font = SYSTEMFONT(15);
            [rightValueTextView sizeToFit];
            [rightValueTextView setHeight:rightValueTextView.height];
            
            UITextView *analyzeTextView = [[UITextView alloc] initWithFrame:CGRectMake(15 + 54, MAX(20, rightValueTextView.bottom), MainScreenWidth - 100 - 69 - 15, 20)];
            analyzeTextView.attributedText = [[NSAttributedString alloc] initWithAttributedString:modelxxx.analyzeMutable];
            analyzeTextView.font = SYSTEMFONT(15);
            [analyzeTextView sizeToFit];
            [analyzeTextView setHeight:analyzeTextView.height];
            return MAX((MAX(20, rightValueTextView.bottom) + 7 + 20), analyzeTextView.bottom) + 12;
        } else {
            return 1;
        }
    }
...
- (void)courseExamPopCellgapfillingChooseStatusChanged:(CourseExamPopCell *)answerCell button:(nonnull UIButton *)sender {
...
- (void)getData {
    if (SWNOTEmptyStr(_examIds)) {
        ...
        if ([_examType isEqualToString:@"1"]) {
            ...
            [param setObject:_examNewSelectCount forKey:@"topic_num"];
...
- (void)getExamDetailForExamIds:(NSString *)examIds {
    if (SWNOTEmptyStr(examIds)) {
        ShowHud(@"试题信息拉取中...");
        [Net_API requestGETSuperAPIWithURLStr:[Net_Path courseExamPopInfoNet:examIds] WithAuthorization:nil paramDic:SWNOTEmptyStr(_course_classId) ? @{@"class_id":_course_classId} : nil finish:^(id  _Nonnull responseObject) {
            if (SWNOTEmptyDictionary(responseObject)) {
                if ([[responseObject objectForKey:@"code"] integerValue]) {
                    NSMutableArray *passArray = [NSMutableArray arrayWithArray:[ExamDetailModel mj_objectArrayWithKeyValuesArray:responseObject[@"data"][@"topic"]]];
                    self.currentExamInfoDict = [NSDictionary dictionaryWithDictionary:responseObject[@"data"]];
                    [self makeBottomView];
                    ...
                    dispatch_group_notify(group, dispatch_get_main_queue(), ^{
                        if (SWNOTEmptyArr(passArray)) {
                            ...
                            self->currentExamId = [NSString stringWithFormat:@"%@",((ExamDetailModel *)passArray[0]).examDetailId];
...
- (void)makeUIByExamDetailModel:(id)examModel {
    if (examModel) {
        ...
        UILabel *examThemeLabel = [[UILabel alloc] initWithFrame:CGRectMake(15, 0, MainScreenWidth-15 - 14 - 53, 0.01)];
        ...
        if ([model.question_type isEqualToString:@"7"]) {
            ...
            if (SWNOTEmptyArr(model.topics)) {
                ...
                // 当前试题只有一道题 就不需要这个tableheader 设置高度0.01 不能设置成0 不然会自动适配一个35高度的空白 并设置 tableview 的 header
                [_headerView setHeight:0.01];
                examThemeLabel.hidden = YES;
                _tableView.tableHeaderView = _headerView;
            }
        }
        [_tableView reloadData];
...
- (void)bottomButtonClick:(UIButton *)sender {
    if (sender == _nextExamBtn) {
        ...
        // 直接判断是否正确
        // 后台设置答错继续答题，按钮为“下一题” 未设置继续答题按钮为“我知道了，继续学习”
         ...
         */
        ExamDetailModel *model = [self checkExamDetailArray:currentExamId];
        NSString *answer_pass = [NSString stringWithFormat:@"%@",self.currentExamInfoDict[@"answer_pass"]];
        NSString *answer_max_count_string = [NSString stringWithFormat:@"%@",self.currentExamInfoDict[@"answer_max_count"]];
        BOOL is_right_answer = [self judgeCurrentExamIsRight];
        if (model.is_answer) {
            // 已经作答了 这里应该是下一题
            if ([answer_pass isEqualToString:@"1"] || ([answer_max_count_string integerValue] == examNum)) {
                // 继续学习 但是这里如果答错 需要展示解析 并且按钮变成“我知道了，继续学习” 点击后继续学习
                // 离开页面
//                [self.navigationController popViewControllerAnimated:YES];
                __weak CourseExamPopViewController *wekself = self;
                wekself.examPopSucceseResult(YES);
                return;
            } else {
                // 继续请求接口获得最新试题 作答 直到答对或者答题数量达到后台配置的数量才能继续作答
                [self getExamDetailForExamIds:_course_ID];
                return;
            }
            return;
...
//                [self.navigationController popViewControllerAnimated:YES];
                ...
                if ([answer_pass isEqualToString:@"1"] || ([answer_max_count_string integerValue] == examNum)) {
                    // 继续学习 但是这里如果答错 需要展示解析 并且按钮变成“我知道了，继续学习” 点击后继续学习
                    [_nextExamBtn setTitle:@"我知道了，继续学习" forState:0];
                    [_nextExamBtn setImage:nil forState:0];
                    [EdulineV5_Tool dealButtonImageAndTitleUI:_nextExamBtn];
                    return;
                } else {
                    ...
                    [EdulineV5_Tool dealButtonImageAndTitleUI:_nextExamBtn];
                    // 继续请求接口获得最新试题 作答 直到答对或者答题数量达到后台配置的数量才能继续作答
//                    [self getExamDetailForExamIds:@"342"];
                    return;
                }
            }
        }
...
- (void)courseExamPopCelluserTextFieldChange:(NSNotification *)notice {
    UITextField *textF = (UITextField *)notice.object;
    CourseExamPopCell *cell = (CourseExamPopCell *)textF.superview.superview;
...
// MARK: - 输入框通知(textView)
- (void)courseExamPopCelluserTextViewChange:(NSNotification *)notice {
