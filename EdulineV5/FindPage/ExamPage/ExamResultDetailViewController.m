...
@implementation ExamResultDetailViewController
...
- (void)makeTopView {
    ...
    _examSheetBtn = [[UIButton alloc] initWithFrame:CGRectMake(MainScreenWidth - 25 - 20, 0, 20, 20)];
...
- (void)getData {
    ...
    if (SWNOTEmptyDictionary(_paperInfo)) {
        ...
        for (int i = 0; i<_examIdListArray.count; i++) {
            ExamSheetModel *currentExamIdListModel = _examIdListArray[i];
            for (int j = 0; j<currentExamIdListModel.child.count; j++) {
                // 这个地方有可能会出现因为题型部分里面没有试题导致崩溃
                ExamModel *idModel = currentExamIdListModel.child[j];
...
- (void)getExamDetailForExamIds:(ExamModel *)examIds {
    if (SWNOTEmptyStr(examIds.topic_id)) {
        ...
        NSString *getUrl = [Net_Path examResultTestDetailInfoNet];
            ...
            if (SWNOTEmptyDictionary(responseObject)) {
                if ([[responseObject objectForKey:@"code"] integerValue]) {
                    ...
                    for (int i = 0; i<passArray.count; i++) {
                        ...
                        pass.is_right = pass.answer_right;
                        pass.is_answer = YES;
                        pass.is_expand = YES;
...
- (void)makeUIByExamDetailModel:(id)examModel {
    if (examModel) {
        ...
        if (_currentExamPaperDetailModel) {
            examThemeLabel.text = _currentExamPaperDetailModel.paper_title;
        }
...
- (void)bottomButtonClick:(UIButton *)sender {
    if (sender == _nextExamBtn) {
        ...
        if (SWNOTEmptyArr(_examIdListArray)) {
            ExamSheetModel *idListModel = _examIdListArray[currentExamIndexPath.section];
        ...
        if (SWNOTEmptyArr(_examIdListArray)) {
            ...
            if (currentExamIndexPath.row>0) {
                ...
                if (currentExamIndexPath.section>0) {
                    ExamSheetModel *preModel = _examIdListArray[currentExamIndexPath.section-1];
        ...
        // 跳转到答题卡页面
        ExamResultSheetViewController *vc = [[ExamResultSheetViewController alloc] init];
        vc.sheetTitle = _currentExamPaperDetailModel ? _currentExamPaperDetailModel.paper_title : @"";
        ...
        vc.chooseOtherExam = ^(ExamModel * _Nonnull model) {
            _previousExamBtn.enabled = NO;
            _nextExamBtn.enabled = NO;
            currentExamId = [NSString stringWithFormat:@"%@",model.topic_id];
            NSInteger indexCount = 0;
          // 获取当前点击的试题的ID 请求接口 刷新页面  处理下标 当前试题的排序号 并且改变底部按钮的状态
            ...
            for (int i = 0; i < _examIdListArray.count; i ++) {
                ...
                for (int j = 0; j<idListModel.child.count; j++) {
                    indexCount = indexCount + 1;
                    if ([((ExamModel *)(idListModel.child[j])).topic_id isEqualToString:model.topic_id]) {
                        is_find = YES;
                        currentExamIndexPath = [NSIndexPath indexPathForRow:j inSection:i];
                        // 这时候判断 并且更改底部按钮状态
                        if (currentExamIndexPath.section == 0 && currentExamIndexPath.row == 0) {
            ...
            [self getExamDetailForExamIds:model];
        };
