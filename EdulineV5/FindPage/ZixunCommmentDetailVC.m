...
@implementation ZixunCommmentDetailVC
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        cell.delegate = self;
        [cell setCommentInfo:_topCellInfo showAllContent:NO];
        cell.commentCountButton.hidden = YES;
        cell.editButton.hidden = YES;
        return cell;
    } else {
        static NSString *reuse = @"ZixunCommentReplayCell";
        ZixunCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:reuse];
        if (!cell) {
            cell = [[ZixunCommentCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuse cellType:NO];
        }
...
- (void)getCommentReplayList {
    ...
    if (SWNOTEmptyStr(_commentId)) {
        ...
        [Net_API requestGETSuperAPIWithURLStr:[Net_Path zixunPostCommentReplay:_commentId] WithAuthorization:nil paramDic:param finish:^(id  _Nonnull responseObject) {
