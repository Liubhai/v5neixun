...
@interface ShopSearchVC ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource> {
    BOOL shouldLoad;
}
...
- (void)jumpSearchListMainPage {
    ...
    if (!SWNOTEmptyStr(_institutionSearch.text)) {
        [self showHudInView:self.view showHint:@"请输入搜索商品的名字"];
        return;
    }
    // 把搜索历史数据存本地
    ...
    [[NSUserDefaults standardUserDefaults] setObject:pass forKey:@"shopSearchArray"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    
    ShopSearchResultListVC *vc = [[ShopSearchResultListVC alloc] init];
...
- (void)searchHotSearchInfo {
    [Net_API requestGETSuperAPIWithURLStr:[Net_Path shopHotSearch] WithAuthorization:nil paramDic:nil finish:^(id  _Nonnull responseObject) {
        if (SWNOTEmptyDictionary(responseObject)) {
            [_hotDataSource removeAllObjects];
            [_hotDataSource addObjectsFromArray:[responseObject objectForKey:@"data"]];
            [_searchDataSource removeAllObjects];
            if ([[NSUserDefaults standardUserDefaults] objectForKey:@"shopSearchArray"]) {
                [_searchDataSource addObjectsFromArray:[[NSUserDefaults standardUserDefaults] objectForKey:@"shopSearchArray"]];
            }
            [self makeScrollViewUI];
        }
