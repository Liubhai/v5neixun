...
@interface UserHomePageViewController ()<UIScrollViewDelegate,UITextViewDelegate> {
    NSInteger wordMax;
}
...
//    _questionListVC.teacherId = _teacherId;
//    _questionListVC.tabelHeight = _mainScrollView.height;
    _questionListVC.view.frame = CGRectMake(MainScreenWidth*2,0, MainScreenWidth, _mainScrollView.height);
    [_mainScrollView addSubview:_questionListVC.view];
    [self addChildViewController:_questionListVC];
    ...
    _userCircleListVC.view.frame = CGRectMake(0,0, MainScreenWidth, _mainScrollView.height);
    ...
    _teacherCourseVC.isUserHomePage = YES;
...
- (void)popButtonClick:(UIButton *)sender {
    ...
    if (sender == _popCancelButton) {
        ...
        [Net_API requestPOSTWithURLStr:[Net_Path askQuestionNet] WithAuthorization:nil paramDic:@{@"recive_user_id":[NSString stringWithFormat:@"%@",_teacherInfoDict[@"id"]],@"content":_popTextView.text} finish:^(id  _Nonnull responseObject) {
            if (SWNOTEmptyDictionary(responseObject)) {
