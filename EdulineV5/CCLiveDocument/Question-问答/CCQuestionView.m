...
-(QuestionTextField *)questionTextField {
    if(!_questionTextField) {
        ...
        _questionTextField.layer.cornerRadius = 21;
...
-(UIButton *)leftView {
    if(!_leftView) {
        ...
        _leftView.frame = CGRectMake(0, 0, 45, 42);
...
-(void)leftButtonClicked {
    ...
    [self.imageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(self.contentView.mas_top).mas_equalTo(-3);
        make.size.mas_equalTo(CGSizeMake(117, 30));
    }];
...
-(UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, tableView.frame.size.width, 13)];
...
-(CGFloat)heightForCellOfQuestion:(NSMutableArray *)array {
    ...
    if([array count] == 1) {
        if(textSize.height < 20) {
            ...
            height = 35 + textSize.height + 10+5;
        };
    } else {
        height = 35 + 10 + textSize.height + 8;
