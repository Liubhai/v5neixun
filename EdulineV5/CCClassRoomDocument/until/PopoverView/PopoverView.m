...
static float kPopoverViewCellHeight = 90.f; ///< cell指定高度
...
@property (nonatomic, copy, readwrite) PopverViewHideBlock hideblock;
@end
...
- (void)showToPoint:(CGPoint)toPoint {
    ...
    CGFloat currentH = 0.f;
    PopoverAction *action = [_actions firstObject];
    if (_actions.count == 1 && action.showView)
    {
        kPopoverViewCellHeight = 90.f;
        currentW= [self calculateMaxWidth];
        _tableView.userInteractionEnabled = NO;
        if (action.showView)
        {
            [_tableView removeFromSuperview];
            currentH = action.showView.frame.size.width;
            [self addSubview:action.showView];
            __weak typeof(self) weakSelf = self;
            [action.showView mas_makeConstraints:^(MASConstraintMaker *make) {
//                make.centerX.mas_equalTo(weakSelf);
//                make.centerY.mas_equalTo(weakSelf.mas_centerX).offset(kPopoverViewArrowHeight/2.f);
                if (!weakSelf.isUpward)
                {
                ...
                make.width.mas_equalTo(action.showView.frame.size.width);
                make.height.mas_equalTo(action.showView.frame.size.height);
            }];
        }
    }
    else
    {
        kPopoverViewCellHeight = 30.f;
        currentW = [self calculateMaxWidth];
        // 刷新数据以获取具体的ContentSize
        [_tableView reloadData];
        // 根据刷新后的ContentSize和箭头指向方向来设置当前视图的frame
        currentH = _tableView.contentSize.height + kPopoverViewArrowHeight;
        // 限制最高高度, 免得选项太多时超出屏幕
...
- (CGFloat)calculateMaxWidth {
    ...
    for (PopoverAction *action in _actions) {
        if (action.showView)
        {
            maxWidth = ceil(action.showView.frame.size.width);
        }
        else
        {
            imageWidth = 0.f;
...
- (void)showToView:(UIView *)pointView withActions:(NSArray<PopoverAction *> *)actions hideBlock:(PopverViewHideBlock)block
{
