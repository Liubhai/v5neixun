...
- (void)drawRect:(CGRect)rect {
    ...
    if (_showProgressText) {
        ...
        //字体
        UIFont *font = [UIFont systemFontOfSize:FontSizeClass_36];
        //构建属性集合
        NSDictionary *attributes = @{NSFontAttributeName:font, NSParagraphStyleAttributeName:textStyle, NSForegroundColorAttributeName:self.pathFillColor};
        //获得size
        CGSize stringSize = [currentText sizeWithAttributes:attributes];
        //垂直居中
        CGRect r = CGRectMake((CircleSelfWidth-stringSize.width)/2.0, (CircleSelfHeight - stringSize.height)/2.0 - 10,stringSize.width, stringSize.height);
        [currentText drawInRect:r withAttributes:attributes];
        
        {
            //画文字
            NSString *currentText = HDClassLocalizeString(@"签到率") ;
            //段落格式
            NSMutableParagraphStyle *textStyle = [[NSMutableParagraphStyle defaultParagraphStyle] mutableCopy];
            textStyle.lineBreakMode = NSLineBreakByWordWrapping;
            textStyle.alignment = NSTextAlignmentCenter;//水平居中
            ...
            CGSize stringSize = [currentText sizeWithAttributes:attributes];
            //垂直居中
            CGRect r1 = CGRectMake((CircleSelfWidth-stringSize.width)/2.0, CGRectGetMaxY(r) + 5.f,stringSize.width, stringSize.height);
            [currentText drawInRect:r1 withAttributes:attributes];
        }
    }
