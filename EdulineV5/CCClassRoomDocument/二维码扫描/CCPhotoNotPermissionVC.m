...
@interface CCPhotoNotPermissionVC ()
...
@implementation CCPhotoNotPermissionVC
...
-(UIBarButtonItem *)rightBarCancelBtn {
    if(_rightBarCancelBtn == nil) {
        _rightBarCancelBtn = [[UIBarButtonItem alloc] initWithTitle:HDClassLocalizeString(@"取消") style:UIBarButtonItemStylePlain target:self action:@selector(onCancelBtn)];
        [_rightBarCancelBtn setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:FontSizeClass_16],NSFontAttributeName,[UIColor whiteColor],NSForegroundColorAttributeName, nil] forState:UIControlStateNormal];
    }
...
-(UILabel *)inforLabel {
    if(!_inforLabel) {
        ...
        NSMutableAttributedString *attrStr = [[NSMutableAttributedString alloc] initWithString:HDClassLocalizeString(@"请在iPhone的“设置-隐私-照片”选项中，\n允许CC云直播访问你的手机相册。") ];
        NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc]init];
        [paragraphStyle setLineSpacing:CCGetRealFromPt(18)];
        paragraphStyle.alignment = NSTextAlignmentCenter;
        NSDictionary *dict = @{NSFontAttributeName:[UIFont systemFontOfSize:FontSizeClass_13],NSParagraphStyleAttributeName:paragraphStyle};
