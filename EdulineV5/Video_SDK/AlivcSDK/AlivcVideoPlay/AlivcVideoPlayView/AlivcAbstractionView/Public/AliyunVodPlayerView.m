...
//            if (_trackInfoArray.count) {
//                info.trackDefinition = [NSString stringWithFormat:@"%@",_trackInfoArray[0][@"definition"]];
//                info.vodPlayUrl = [NSString stringWithFormat:@"%@",_trackInfoArray[0][@"play_url"]];
//            }
//            @[@"FD",@"HD",@"LD",@"OD",@"SD",@"2K",@"4K",@"SQ",@"HQ"]
//            @[@"FD",@"HD",@"LD",@"OD",@"SD",@"2K",@"4K",@"SQ",@"HQ"]
//            NSArray *array = @[@"FD",@"HD",@"LD",@"OD",@"SD",@"2K",@"4K",@"SQ",@"HQ"];
            for (int i = 0; i < _trackInfoArray.count; i++) {
                AVPTrackInfo * infoModel = [[AVPTrackInfo alloc] init];
                infoModel.trackType = AVPTRACK_TYPE_SAAS_VOD;
                infoModel.trackDefinition = [NSString stringWithFormat:@"%@",_trackInfoArray[i][@"definition"]];
                infoModel.vodPlayUrl = [NSString stringWithFormat:@"%@",_trackInfoArray[i][@"play_url"]];
                infoModel.trackIndex = i;//[array indexOfObject:infoModel.trackDefinition];
                [trackArray addObject:infoModel];
            }
            AVPMediaInfo *mediaInfo = [[AVPMediaInfo alloc] init];
            mediaInfo.tracks = [NSArray arrayWithArray:trackArray];
            if (trackArray.count) {
                _currentTrackInfo = trackArray[_trackInfoIndex];
                [self.controlView setBottomViewTrackInfo:trackArray[_trackInfoIndex]];
            }
            
            if (self.playMethod == ALYPVPlayMethodUrl) {
                [self updateControlLayerDataWithMediaInfo:mediaInfo];
            }else{
        ...
        case AVPEventFirstRenderedStart: {
            ...
            if (self.coverImageView) {
                if (_currentTrackInfo) {
                    if ([_currentTrackInfo.trackDefinition isEqualToString:@"SQ"] || [_currentTrackInfo.trackDefinition isEqualToString:@"HQ"]) {
                        self.coverImageView.hidden = NO;
                    } else {
...
- (void)aliyunControlView:(AliyunPlayerViewControlView *)controlView qualityListViewOnItemClick:(int)index{
    ...
    //切换清晰度
//    [self.aliPlayer selectTrack:index accurate:YES];
    _trackInfoIndex = index;
    NSTimeInterval currentTimeTemp = self.aliPlayer.currentPosition;
    [self playViewPrepareWithURL:[NSURL URLWithString:self.trackInfoArray[index][@"play_url"]]];
    [self.aliPlayer seekToTime:currentTimeTemp seekMode:AVP_SEEKMODE_ACCURATE];
}
