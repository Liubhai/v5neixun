...
- (UILabel *)fullScreenTimeLabel{
    if (!_fullScreenTimeLabel) {
        ...
        [str addAttribute:NSForegroundColorAttributeName value:kALYPVColorTextGray range:NSMakeRange(curTimeStr.length, totalTimeStr.length + 1)];
...
- (void)layoutSubviews{
    ...
    if (self.isPortrait) {
        ...
        self.progressView.frame = CGRectMake(ALYPVBottomViewPlayButtonWidth + isPortraitTimeWidth, height,
                                             width - (ALYPVBottomViewPlayButtonWidth + ALYPVBottomViewFullScreenButtonWidth+qualityWidth) - isPortraitTimeWidth * 2,
                                             height);
        
        CGRect progressFrame = self.progressView.frame;
        self.leftTimeLabel.frame = CGRectMake(ALYPVBottomViewPlayButtonWidth, 0, isPortraitTimeWidth, height);
        self.rightTimeLabel.frame = CGRectMake(progressFrame.origin.x + progressFrame.size.width,0, isPortraitTimeWidth, height);
        if (@available(iOS 16, *)) {
        } else {
            return;
        }
    ...
    if ([AliyunUtil isInterfaceOrientationPortrait]) {
        ...
        self.progressView.frame = CGRectMake(ALYPVBottomViewPlayButtonWidth + isPortraitTimeWidth, 0,
                                             width - (ALYPVBottomViewPlayButtonWidth + ALYPVBottomViewFullScreenButtonWidth) - isPortraitTimeWidth * 2,
                                             height);
...
//    [self.fullScreenButton setBackgroundImage:[AliyunUtil imageWithNameInBundle:@"al_play_screen_full" skin:skin] forState:UIControlStateNormal];
    ...
    [self.qualityButton setBackgroundImage:[AliyunUtil imageWithNameInBundle:@"al_quality_btn_nomal" skin:skin] forState:UIControlStateSelected];
